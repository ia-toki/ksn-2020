...
vector<vector<int>> sevenGrouping = {
  {3, 4, 5, 6, 8, 10, 12},
  {3, 4, 5, 6, 7, 10, 11},
  {3, 4, 5, 6, 7, 8, 9},
  {0, 1, 5, 6, 8, 10, 12},
...
vector<vector<int>> getGroupsWithBalls(vector<bool> ans, int ngroup, vector<vector<int>> grouping, vector<vector<int>> groups) {
  for (int i = 0; i < ngroup; ++i) {
...
vector<vector<int>> getGroupsWithBalls(vector<bool> ans, int ngroup, vector<vector<int>> grouping1, vector<vector<int>> groups1, vector<vector<int>> grouping2, vector<vector<int>> groups2) {
  int cnt = 0;
  for (int i = 0; i < ngroup; ++i) {
    for (int j = 0; j < ngroup; ++j) {
      bool okay = true;
      for (int k = 0; k < ans.size(); ++k) {
        bool expect = binary_search(grouping1[k].begin(), grouping1[k].end(), i) ||
                      binary_search(grouping2[k].begin(), grouping2[k].end(), j);
        if (expect != ans[k]) {
          okay = false;
          break;
        }
      ...
      if (okay) return {groups1[i], groups2[j]};
    }
  }
...
vector<vector<int>> findGroups(int ngroup, vector<vector<int>> grouping, vector<int> balls1, vector<int> balls2) {
  vector<vector<int>> grouping1, grouping2;
  for (int i = 0; i < grouping.size(); ++i) {
    grouping1.push_back(vector<int>()); grouping2.push_back(vector<int>());
    for (int j = 0; j < grouping[i].size(); ++j) {
      if (grouping[i][j] < ngroup) grouping1.back().push_back(grouping[i][j]);
      else grouping2.back().push_back(grouping[i][j] - ngroup);
    }
  }

  vector<vector<int>> groups1 = getGrouping(ngroup, grouping1, balls1);
  vector<vector<int>> boxesSets1 = getBoxesSetsFromGrouping(groups1, grouping1);
  
  vector<vector<int>> groups2 = getGrouping(ngroup, grouping2, balls2);
  vector<vector<int>> boxesSets2 = getBoxesSetsFromGrouping(groups2, grouping2);
  
  vector<vector<int>> boxesSets;
  for (int i = 0; i < grouping.size(); ++i) {
    boxesSets.push_back(boxesSets1[i]);
    boxesSets.back().insert(boxesSets.back().end(), boxesSets2[i].begin(), boxesSets2[i].end());
  }
  extendQueriesWithNoBalls(boxesSets);

  vector<bool> ans = ask(boxesSets);

  return getGroupsWithBalls(ans, ngroup, grouping1, groups1, grouping2, groups2);
}
...
int main() {
  ...
  if (k == 1) {
    ...
    if (ballGroups.size() == 1) {
      ...
      ballGroups = findGroups(7, sevenGrouping, ballGroups[0], ballGroups[1]);
      updatePossibleBalls(join(ballGroups));
      answer(binary(5, ballGroups[0], ballGroups[1]));
    }
  }
